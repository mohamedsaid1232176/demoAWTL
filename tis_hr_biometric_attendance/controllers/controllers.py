...
class ZKTecoController(http.Controller):
    ...
    @http.route(['/iclock/cdata','/iclock/cdata.aspx'], type='http', auth='public', methods=['GET'])
    def zkteco_cdata(self, **kwargs):
        ...
        device_id = request.env['biometric.config'].sudo().search([
            ('serial_number','=',sn)])
        if device_id:
            device_id.last_update=fields.Datetime.now()
    ...
    @http.route(['/iclock/cdata', '/iclock/cdata.aspx'], type='http', auth='none', methods=['POST'],
                csrf=False)
    ...
    @http.route(["/iclock/getrequest", "/iclock/getrequest.aspx"], type='http', auth='public',
    ...
    @http.route(['/iclock/devicecmd','/iclock/devicecmd.aspx'], type='http', auth='public',
    ...
    def device_command(self, **kwargs):
        ...
        for line in raw_data.split('\n'):
            ...
            if (param_dict.get("CMD") == "DATA" or param_dict.get("CMD") == "CHECK"
                    or param_dict.get("CMD") =="REBOOT") or param_dict.get("CMD") =="ENROLL_FP":
