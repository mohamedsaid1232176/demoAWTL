...
class BiometricDeviceConfig(models.Model):
    ...
    tz_num = fields.Char(
        string='Timezone',
        compute='_compute_tz_num',
        store=True
    )
    used_for = fields.Selection([('both', "Both"),
                                 ("check_in", "Check-In"),
                                 ("check_out", "Check-Out")
                                 ], default="both", required=True,
                                help="This field is used to determine whether "
                                     "the device is used for Check-In, "
                                     "Check-Out, or both.")
    is_status_number_based = fields.Boolean("Status Code Based",
                                            help="When enabled, the system will use the status codes defined "
                                                 "in Biometric Config → Attendance Status to generate attendance"
                                                 " logs accordingly.")
    ...
    def _compute_tz_num(self):
        for r in self:
            try:
                timezone = pytz.timezone(r.time_zone or 'GMT')
                now_in_tz = datetime.now(timezone)
                offset_hours = now_in_tz.utcoffset().total_seconds() / 3600

                # If there is a decimal part, multiply by 60
                if float(offset_hours).is_integer():
                    r.tz_num = str(int(offset_hours))
                else:
                    r.tz_num = str(int(offset_hours * 60))

            except Exception:
                # Fallback to GMT if invalid timezone
                r.tz_num = "0"
    ...
    def download_attendance_log(self):
        if self.is_adms:
            return {
                'name': 'Download ADMS Attendance',
                'type': 'ir.actions.act_window',
                'res_model': 'attendance.adms.download.wizard',
                'view_mode': 'form',
                'target': 'new',
                'context': {
                    ...
                    if attendances:
                        ...
                        for entry in sorted_attendance:
                            ...
                            atten_time = datetime.strptime(entry['atten_time'],
                                                           '%Y-%m-%d %H:%M:%S')
                            if not employee_status[device_user_id]:
                                employee_status[device_user_id].append(
                                ...
                                if atten_time > employee_status[device_user_id][-1]['time']:
                                    current_status = 'Check-out' if employee_status[device_user_id][-1]['status'] == 'Check-in' else 'Check-in'
                                    employee_status[device_user_id].append(
                                ...
                                for log in existing_log:
                                    if log.is_calculated == True:
                                        if entry['status'] == 'Check-in':
                                            ...
                                            attendance_status = '1'
                                        if self.used_for == "check_in":
                                            attendance_status = '0'
                                        elif self.used_for == "check_out":
                                            attendance_status = '1'
                                        vals = {
                                            'employee_id': employee_id,
                                            'punching_time': entry_time,
 ...
 # Adjust if necessary (e.g., if device name is used for logs)
    ...
    def create_device_user(self, values):
        ...
        device_user_id = self.env['biometric.attendance.devices'].sudo().search([
    ...
    def create_att_log(self, raw_data):
        for line in raw_data.splitlines():
            ...
            number = values[4]
            db_user_device_id = self.env['biometric.attendance.devices'].sudo().search([
                ('biometric_attendance_id', '=', user_device_id),
                ('device_id', '=', self.id)])
            ...
            att_status = False
            if self.used_for == "both":
                if self.is_status_number_based:
                    if att_status_id:
                    ...
                    last_log_ids = self.env['attendance.log'].search([('device_user_id', '=', db_user_device_id.id)]).sorted('punching_time')
                    if last_log_ids:
                        last_log_id = last_log_ids[-1]
                        if last_log_id.status == "0":
                            att_status = "1"
                        elif last_log_id.status == "1":
                            att_status = "0"
                        else:
                            att_status = "2"
                    else:
    ...
    def create_finger_print(self, values):
        ...
        f_id = values[2].split('=')[1]
        f_size = values[3].split('=')[1]
        ...
        fp_exist = self.env['finger.template'].sudo().search([('device_id', '=', self.id),
                                                       ('device_user_id', '=', db_user_device_id.id),('f_id','=',f_id)])
        if fp_exist:
            fp_exist.template_data = tmp
            fp_exist.f_id = f_id
            fp_exist.f_size = f_size
        else:
            fp_id = self.env[
            ...
            if command_exist:
                if command_exist.employee_id and command_exist.name == "DATA":
                    ...
                    command_exist.status = 'success'
                      ...
                      or command_exist.name == "ATTLOG" or command_exist.name == "REBOOT" or command_exist.name == "ENROLL_FP"):
                    command_exist.status = 'success'
    ...
    def export_employee(self):
        ...
        for employee_id in employee_ids:
            device_user = employee_id.biometric_device_ids.filtered(
                lambda x: x.device_id == self)
            if not device_user:
                ...
                if conn:
                    ...
                    for user in users:
                        ...
                        if not user_exist:
                            ...
                            user_exist.device_user_name = user.name
                    return {'name': 'Success Message',
                     'type': 'ir.actions.act_window',
                     'res_model': 'success.wizard',
                     'view_mode': 'form',
    ...
    def action_refresh_connection(self):
        """ Refreshes the connection state of the biometric device."""
        for record in self:
            if record.last_update:
                time_diff = fields.Datetime.now() - record.last_update
                if time_diff > timedelta(minutes=1):
                    record.state = 'not_connected'
                    record.check_connection()


    def action_restart_connection(self):
        """ Restarts the biometric device."""
        for record in self:
            if record.is_adms:
                return {
                    'type': 'ir.actions.act_window',
                    'res_model': 'restart.button.wizard',
                    'view_mode': 'form',
                    'target': 'new',
                    'context': {
                        'default_device_id': record.id,
                    }
                }
            else:
                zk = ZK(record.device_ip, record.port, password=record.device_password)
                try:
                    conn = zk.connect()
                    if zk.restart():
                        print(f"Device {record.device_ip} restarted successfully.")
                    else:
                        print(f"Failed to restart device {record.device_ip}.")
                    conn.disconnect()
                except Exception as e:
                    print(f"Error during restart of device {record.device_ip}: {e}")
