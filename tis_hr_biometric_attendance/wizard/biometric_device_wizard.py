...
class BiometricDeviceWizard(models.TransientModel):
    ...
    def confirm_update_with_biometric(self):
        ...
        if self.biometric_id.is_adms:
            device_user_id = self.env['biometric.attendance.devices'].search([
                ('employee_id', '=', employee.id),
                ('device_id', '=', self.biometric_id.id),
            ])
            if not device_user_id:
                employee.create_export_command(self.biometric_id)
            else:
                employee.update_export_command(self.biometric_id)
        else:
                ...
                if users:
                    ...
                    if valid == True:
                        ...
                        if biometric_attendance_id:
                            zk.set_user(int(biometric_attendance_id),
                                        employee.name, 0, '', '',
                                        biometric_attendance_id)
                        else:
    ...
    def remove_employee_from_biometric(self):
        ...
        if self.biometric_id.is_adms:
            ...
            if device_user_id:
                employee.employee_del_command(self.biometric_id)
            else:
                raise ValidationError("The user does not exist on this device")
        else:
            biometric_attendance_id = ''
            for attendance_id in employee.biometric_device_ids:
                biometric_attendance_id = attendance_id.biometric_attendance_id
